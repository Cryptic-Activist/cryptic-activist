// buildFirstTradeRewardEmail.ts
import { FRONTEND_PUBLIC } from '@/constants/env';

const buildFirstTradeWithReferralReferee = (
  user: any,
  trade: any,
  xpAmount: number,
) => {
    ...
    <tr>
      <td style="padding: 30px 40px;">
        ...
        <p style="margin:0 0 30px;color:#333333;font-size:16px;line-height:24px;">
          Your first trade (ID: <strong>${trade.id}</strong>) was successfully executed on <strong>${new Date(trade.timestamp).toLocaleString()}</strong>.
        </p>

        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:30px;">
          <tr>
            <td style="padding:8px 0;font-size:15px;color:#333;"><strong>Crypto Traded:</strong></td>
            <td style="padding:8px 0;font-size:15px;color:#333;">${trade.cryptoAmount} ${trade.crypto}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;font-size:15px;color:#333;"><strong>Fiat Spent:</strong></td>
            <td style="padding:8px 0;font-size:15px;color:#333;">${trade.fiatAmount} ${trade.fiat}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;font-size:15px;color:#333;"><strong>Reward Earned:</strong></td>
...
export default buildFirstTradeWithReferralReferee;
